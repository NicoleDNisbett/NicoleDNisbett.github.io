...
get_hours_fb=function(file, debate="debate"){
  timeline = data.frame(strptime(file$`Date / Time`, "%Y-%m-%d %H:%M"))
...
get_dates_fb=function(file, debate){
  timeline = data.frame(strptime(file$`Date / Time`, "%Y-%m-%d %H:%M"))
  fbdates = data.frame(format(timeline, "%Y-%m-%d"))

  fbdates = data.frame(table(fbdates))
  fbdates[,1]=as.Date(fbdates[,1], format="%Y-%m-%d")
...
get_ud_data=function(comments){
  ...
  x=udpipe_annotate(ud_model, x = comments$message, doc_id = comments$document)
  x <- as.data.frame(x)
  return(x)
}
...
display_bigrams = function(tibble, min, debate){
  for (line in (tibble)){
    ngrams=(tibble %>% unnest_tokens(bigrams, message, token="ngrams", n=2, to_lower = TRUE))
  ...
  net = (bigrams_ordered %>% 
           ...
           scale_edge_colour_gradient(high = "#003300", low = "#00FF00")+
           ggtitle(debate)+
...
plot_each_afinn_doc_distributions=function(frequencies, debate){
  ggplot(data= as.data.frame(table(frequencies$score)/nrow(frequencies)*100), aes(x=Var1, y=Freq, fill=as.numeric(Var1))) + 
    geom_bar(stat='identity') +
...
display_afinn_comments=function(df,debate){
  ...
  print(table(qrange$score)/nrow(qrange))
  ...
  qrange %>%
    ...
    ggplot(aes(comment, percentage, fill = score, label=message)) +
    geom_col(show.legend = FALSE) +
